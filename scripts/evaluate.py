...
from scripts.train_pointnet import MLPNet, RaveledDataset
...
def main():
    ...
    model = MLPNet(input_dim=args.input_dim, return_features=True).to(device)
    ...
    with torch.no_grad():
        for r in records:
            pts = torch.tensor(np.load(r["path"]).astype(np.float32)).ravel().unsqueeze(0).to(device)
